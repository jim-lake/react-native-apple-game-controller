...
function ToggleButton({ label, onToggle }: { label: string; onToggle: (v: boolean) => void }) {
  const [on, setOn] = useState(false);
  return (
    <View style={styles.toggleRow}>
      <Text style={styles.toggleLabel}>{label}: {on ? 'ON' : 'OFF'}</Text>
      <Pressable style={[styles.btn, on && styles.btnOn]} onPress={() => {
        const next = !on;
        setOn(next);
        try {
    ...
    try {
      const subs = [
        GameController.onControllerConnected(id => {
          ...
          try {
            GameController.getControllers()
              .then(c => { addLine(`[connect] Refreshed controllers: ${c.length}`); setControllers(c); })
              .catch((e: any) => addLine(`[error] getControllers on connect: ${e.message}`));
          } catch (e: any) {
        ...
        GameController.onControllerDisconnected(id => {
          ...
          try {
            GameController.getControllers()
              .then(c => { addLine(`[disconnect] Refreshed controllers: ${c.length}`); setControllers(c); })
              .catch((e: any) => addLine(`[error] getControllers on disconnect: ${e.message}`));
          } catch (e: any) {
        ...
        GameController.onControllerButton(e => {
          addLine(`[btn] ${e.controllerId.slice(0, 8)} buttons=0x${(e.buttons >>> 0).toString(16)}`);
        }),
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.toggles}>
        <ToggleButton label="ControllerButton" onToggle={v => GameController.toggleControllerButtonEvents(v)} />
        <ToggleButton label="ControllerCurrent" onToggle={v => GameController.toggleControllerCurrentEvents(v)} />
        <ToggleButton label="Keyboard" onToggle={v => GameController.toggleKeyboardEvents(v)} />
        <ToggleButton label="MouseButton" onToggle={v => GameController.toggleMouseButtonEvents(v)} />
        <ToggleButton label="MouseMove" onToggle={v => GameController.toggleMouseMoveEvents(v)} />
      </View>
      ...
      {controllers.map(c => (
        <ControllerStatus key={c.controllerId} controllerId={c.controllerId} pollingEnabled={polling} />
      ))}
...
const styles = StyleSheet.create({
  ...
  btn: { paddingHorizontal: 8, paddingVertical: 4, backgroundColor: '#333', borderRadius: 4 },
